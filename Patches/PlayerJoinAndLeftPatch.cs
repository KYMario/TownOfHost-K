            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                _ = new LateTask(() =>
                {
                    ...
                    if (Main.UseingJapanese)
                    {
                        ...
                        Utils.SendMessage($"{text}{text2}{text3}{text4}", client.Character.PlayerId, $"<{Main.ModColor}>【This Room Use \"Town Of Host-K\"】</color>");
                    }
                    else
                    {
                        var varsion = Main.PluginShowVersion;
                        ...
                        Utils.SendMessage($"{text}{text2}", client.Character.PlayerId, $"<{Main.ModColor}>【This Room Use \"Town Of Host-K\"】</color>");
                    }
                    ...
                    {
                        if (Main.UseingJapanese)
                            Utils.SendMessage($"<size=120%>☆これはデバッグ版です☆</size>\n<line-height=80%><size=70%>\n・正式リリース版ではありません。\n・バグが発生する場合があります。\nバグが発生した場合はTOH-KのDiscordで報告すること!", client.Character.PlayerId, "<#ff1919>【=====　これはデバッグ版です　=====】</color>");
                        else
                            Utils.SendMessage($"<size=120%>☆This is a debug version☆</size=120%>\n<line-height=80%><size=70%>This is not an official release version. \n If you encounter a bug, report it on TOH-K Discord!", client.Character.PlayerId, "<#ff1919>【==　This is Debug version　==】</color>");
                    }
